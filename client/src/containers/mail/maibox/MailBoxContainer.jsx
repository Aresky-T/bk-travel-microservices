...
const MailBoxContainer = ({ isShowMail, handleHiddenMail }) => {
  ...
  const handleSubmitSendMail = async () => {
    dispatch(onLoading());
    try {
      const { email, fullName, title, content } = mail;
      const form = {
        email: email.trim(),
      ...
      if (data) {
        ...
        successAlert(
          "Thành công",
          "Cảm ơn quý khách đã gửi thư tới BK Travel. Chúng tôi sẽ phản hồi tới email của quý khách trong thời gian sớm nhất!",
          "OK"
        ).then(() => {
          dispatch(resetMail());
        });
  ...
  return (
    <CustomPopup className={isShowMail ? "mail-box active" : "mail-box"}>
      <Box
        className="mail-box_header"
        display="flex"
        height="70px"
      ...
      >
        ...
        <Box
          className="mail-box_header__icons"
          display={"flex"}
          width={"fit-content"}
        >
          <CircleButton
      ...
      >
        ...
        {status.isStarted && status.isConnected && (
          <Connected handleSubmitSendMail={handleSubmitSendMail} />
        )}
      </Box>
    </CustomPopup>
  );
